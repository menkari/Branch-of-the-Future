...
var Devices;
$(document).ready(function(){var A=$.getUrlParams();
var C={functions:{setCookie:function(H,D,E){var F="";
if(E){var G=new Date();
G.setTime(G.getTime()+(E*24*60*60*1000));
F=";expires="+G.toGMTString()
}document.cookie=H+"="+D+F+";path=/";
return true
},getCookie:function(F){var D=document.cookie.split(";");
for(x in D){var G=D[x];
var E=$.trim(G).split("=");
if(E[0]==F){return E[1]
}}return null
},deleteCookie:function(D){C.functions.setCookie(D,"",-1)
}}};
Devices={functions:{displayByApp:function(E,F){var D={cookie_key:"westpac.mobile.app",data_display_attr:"data-display-app"};
var F=$.extend(D,F);
if(E){E=E.toLowerCase();
C.functions.setCookie(F.cookie_key,E.toLowerCase())
}else{E=C.functions.getCookie(F.cookie_key)
}if(E){$("["+F.data_display_attr+"='"+E+"']").show().css("display","block")
}},displayByCategory:function(G){var H=["data-display-standard","data-display-iphone","data-display-android"];
var E="data-display-standard";
var I=(G?"data-display-"+G.toLowerCase():E);
var F="data-visible";
for(x in H){var D=H[x];
$("["+H[x]+"]").each(function(){var K=$(this).attr(H[x]);
if(I==D){$(this).attr(F,K)
}else{if(!$(this).attr(F)&&!$(this).attr(I)){var J=$(this).attr(E);
if(J){$(this).attr(F,J)
}else{$(this).attr(F,(K=="true"?"false":"true"))
}}}})
}$("["+F+"='true']").show().css("display","block")
},init:function(){Devices.functions.displayByApp();
Devices.functions.displayByCategory(A.dcat);
if(A.dcat){$("body").addClass(A.dcat.toString().toLowerCase())
}}}};
$("li[data-theme]:has('div[data-listview-item=accordion]') div[data-listview-item=accordion]").accordion();
$("form [data-jquery-plugin='smartradios']").smartRadios();
$("form input[type='checkbox']").persist();
var B={backButton:function(){$("a.back-btn").click(function(D){var E=$(this).attr("href");
if(E==""||E=="#"){if(A.referrer&&A.referrer.toString().toLowerCase()=="app"){location.href="index.html"
}else{history.back()
}D.preventDefault();
return false
}else{return true
}})
},centerHeader:function(){var F=$(".ui-header h1");
if(F.find("b").length==0){F.wrapInner("<b></b>")
}var E=F.find("b");
var D=$(".ui-header .back-btn");
if(D.length>0&&E.width()>($(window).width()-D.width()*2)){F.css({"padding-left":D.width()+5,"padding-right":"10px","text-align":"left"})
}else{F.css({"padding-left":0,"padding-right":"0px","text-align":"center"})
}},roundCorners:function(){$("ul.ui-listview[data-inset='true']").each(function(){var E=$(this).find("li:visible");
var F=E.first();
var D=E.last();
if(F.css("border-top-left-radius")=="0px"){F.addClass("ui-corner-top")
}if(D.css("border-bottom-left-radius")=="0px"){D.addClass("ui-corner-bottom")
}})
},resize:function(){$(window).resize(function(){var D=$(window).height();
var E=$(window).width();
var F=$("html");
if(E>D){F.removeClass("portrait").addClass("landscape")
}else{F.removeClass("landscape").addClass("portrait")
}B.centerHeader()
}).resize()
},fixBrowserSupport:function(){if($("#enquiry_form").length>0){$("#enquiry_form").find("input:not(input[type=hidden]),textarea").convtPlaceholderToLabel()
}},init:function(){Devices.functions.init();
B.resize();
B.backButton();
B.roundCorners();
B.centerHeader();
...
(function(A){A.extend({getUrlParams:function(){var C=[],B;
...
},getUrlParam:function(B){return A.getUrlParameters()[B]
}});
A.fn.extend({accordion:function(D){var C={arrowClass:"icon-arrow-down",arrowExpandedClass:"icon-arrow-up",scrollSpeed:1000,scrollOffset:55};
var D=A.extend(C,D);
var B={init:function(){A("body,html").bind("scroll mousedown DOMMouseScroll mousewheel keyup",function(E){if(E.which>0||E.type==="mousedown"||E.type==="mousewheel"){A("html,body").stop()
}});
A("html,body").bind("touchstart",function(){A("html,body").stop()
})
},toggle:function(F,E){$listItem=F.closest("li");
$arrow=$listItem.find(".ui-li span.ui-icon").first();
if(E==false||F.is(":visible")){B.arrow.collapse($listItem);
F.slideUp("fast")
}else{B.arrow.expand($listItem);
F.slideDown();
A("html,body").animate({scrollTop:F.offset().top-D.scrollOffset},D.scrollSpeed)
}},arrow:{expand:function(E){E.find(".ui-li [class*='icon-arrow']").removeClass(D.arrowClass).removeClass(D.arrowExpandedClass).addClass(D.arrowExpandedClass)
},collapse:function(E){E.find(".ui-li [class*='icon-arrow']").removeClass(D.arrowClass).removeClass(D.arrowExpandedClass).addClass(D.arrowClass)
}}};
B.init();
return this.each(function(){var H=D;
var F=A(this).closest("li");
var G=F.find(".ui-li a.ui-link-inherit");
var E=F.find(".ui-li [class*='icon-arrow']").first();
G.replaceWith(A('<div class="'+G.attr("class")+'" accesskey="'+G.attr("accesskey")+'" tabindex="'+(G.attr("tabindex")?G.attr("tabindex"):"")+'">'+G.html()+"</div>"));
if(A(this).attr("data-accordion-visible")=="true"){B.arrow.expand(F)
}else{B.arrow.collapse(F)
}F.children("div:not('[data-listview-item=accordion]')").unbind("click").click(function(){$list=A(this).closest("ul[data-role='listview']");
$slideContent=A(this).siblings("[data-listview-item='accordion']").first();
if($slideContent.is(":hidden")){var I=$list.find("[data-listview-item='accordion']:visible");
I.each(function(){B.toggle(A(this),false)
});
A.when(I).done(function(){B.toggle($slideContent)
})
}else{B.toggle($slideContent)
}})
})
}});
A.fn.extend({smartRadios:function(D){var C={};
var D=A.extend(C,D);
var B={getInputField:function(E,F){return E.find("input[name='"+F+"']").first()
},getRadioOption:function(E,F){return E.filter("[value='"+F+"']").first()
},setRadioOption:function(E,F,G){F.each(function(){B.unsetRadioOption(A(this))
});
G.attr("checked",true);
G.siblings("label").removeClass("ui-btn-active").addClass("ui-btn-active")
},unsetRadioOption:function(E){var F=E.siblings("label");
E.attr("checked",false);
F.removeClass("ui-btn-active")
},getPopulatedFields:function(F,G){var E=[];
G.each(function(){$field=B.getInputField(F,A(this).val());
if($field&&$field.val()){E.push($field)
}});
return E
}};
return this.each(function(){var I=D;
var G=A(this);
var F=A(this).closest("form");
var H=A(this).find("input[type='radio']");
var E=false;
H.each(function(){var J=A(this);
var L=A(this).siblings("label");
var K=B.getInputField(F,A(this).val());
L.click(function(){B.setRadioOption(F,H,J)
});
if(K){K.keyup(function(){var M=B.getPopulatedFields(F,H);
if((M.length>1)&&showContact){G.show()
}else{G.hide()
}})
}});
if(E){G.show()
...
A.fn.extend({persist:function(D){var C={};
var D=A.extend(C,D);
var B={setCheckbox:function(G){var F=G.siblings(".label").find("span.ui-icon-checkbox");
var E=G.attr("name");
if(E){B.localStorage.set(E,true);
G.attr("checked","checked");
F.removeClass("checked").addClass("checked")
}},unsetCheckbox:function(G){var F=G.siblings(".label").find("span.ui-icon-checkbox");
var E=G.attr("name");
if(E){B.localStorage.remove(E);
G.attr("checked",false);
F.removeClass("checked")
}},localStorage:{get:function(E){if(typeof (localStorage)){return localStorage.getItem(E)
}},set:function(E,F){if(typeof (localStorage)){try{localStorage.setItem(E,F)
}catch(G){if(G==QUOTA_EXCEEDED_ERR){alert("Quota exceeded!")
}}}},remove:function(E){if(typeof (localStorage)){localStorage.removeItem(E)
}}}};
return this.each(function(){var F=A(this).attr("name");
var E=A(this).siblings(".label").find("span.ui-icon-checkbox");
if(F){var G=B.localStorage.get(F);
if(G=="true"){A(this).attr("checked",true);
E.removeClass("checked").addClass("checked")
}}E.click(function(){var H=A(this).closest(".label").siblings("input[type='checkbox']");
if(H.is(":checked")){B.unsetCheckbox(H)
}else{B.setCheckbox(H)
}})
})
}})
})(jQuery);
(function(A){A.fn.labelInputActions=function(){return this.each(function(){var C=A(this),E=C.find("input,textarea"),D=C.find("label"),B=C.find("label").text();
if(E.val()){if(E.val()!==B&&E.val().replace(/\s/g,"")!==""){D.hide()
}}D.bind("click",function(){E.focus()
});
E.bind("blur",function(){if(A(this).val().replace(/\s/g,"")===""){D.show()
}}).bind("focus",function(){D.hide()
})
})
};
A.fn.convtPlaceholderToLabel=function(){return this.each(function(){var E=A(this),C=("placeholder" in document.createElement("input"));
if(!C){var D=A('<label for="'+E.attr("id")+'">'+E.attr("placeholder")+"</label>").insertBefore(E),B=A('<div class="container"></div>').insertAfter(E);
B.append(D).append(E).labelInputActions()
}})
}
})(jQuery);
